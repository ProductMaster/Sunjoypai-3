var img = {
	 ...
	 init:function(){
		...
		img.right_img_btn.onclick = function(){img.play((img.num+1)%img.imagebg_li.length)};
		
		img.autoplay= setInterval(function(){img.play((img.num+1)%img.imagebg_li.length)},5000);
		
		img.imageShow.onmouseover = function(){clearInterval(img.autoplay);}
		img.imageShow.onmouseout = function(){img.autoplay= setInterval(function(){img.play((img.num+1)%img.imagebg_li.length)},5000); }
	...
	play:function(i){
		...
		if(i>img.num){
			...
			if(parseInt(img.current.style.left)>380){
				//如果活动框到了最右边--完成
				//小图片向左
				img.animate = setInterval(function(){
					if(parseInt(img.small_pic.style.left)>(small_pic_left- img.imageNumWidth*(i-img.num)+img.imageNumWidth*(i-img.num)/10)){
						...
						img.small_pic.getElementsByTagName("li")[i].classNadme="currently";
