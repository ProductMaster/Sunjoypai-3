...
<form id="validateForm" class="form-horizontal" method="post" action="${pageContext.request.contextPath}/user/settingUserPic" enctype="multipart/form-data">
	<div class="headPortrait">
		<ul>
			<li>
				<c:if test="${currentUser.picUrl == null  || currentUser.picUrl ==''}">
					<a href="javascript:void(0)" onclick="clickFile()" class="headPortrait-img" >
                   <div id="previewDiv"><img  src="${currentUser.logo}" alt="" width="160" height="160"/></div>
                   </a>
                  </c:if>
                  <c:if test="${currentUser.picUrl != null && currentUser.picUrl != ''}">
                  <a href="javascript:void(0)" onclick="clickFile()" class="headPortrait-img" >
                   <div id="previewDiv"><img src="http://images.sunjoypai.com/user/${photoPath}160X160_${currentUser.picUrl}?rnd=<%=Math.random()%>" alt="" width="160" height="160"/></div>
                   </a>
                  </c:if>
				<p>点击头像更换</p>
