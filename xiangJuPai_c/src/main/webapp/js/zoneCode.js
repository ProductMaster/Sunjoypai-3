...
function initProzoneByProDiv(divID,prozoneNo,cityNo,zoneNo)
{
	$.getScript(basePath+"/sys/initZone?disType=json&"+Math.random()+"&jsoncallback=?",
    	function(data){
    		var selectHtml 	= '<select name="prozoneNo" id="prozoneNo" class="required form-control" title="请选择省份" onchange="initDealer(this);" id="provinceNo">';
			...
			selectHtml += '</select>';//省
			selectHtml += '<select name="customerList" id="customerList" onchange="chooseCustomer(this);" class="required">';
			selectHtml += '<option value="">---请选择---</option></select>';
...
function initProzoneByDIVID(divID,prozoneNo,cityNo,zoneNo)
{
	$.getScript(basePath+"/sys/initZone?disType=json&"+Math.random()+"&jsoncallback=?",
    	function(data){
    		var selectHtml 	= '<select name="prozoneNo" id="prozoneNo" class="required form-control" title="请选择省份" onchange="initCityZoneNo(this);" id="provinceNo">';
    			selectHtml += '<option value="">---请选择---</option>';
    		var json  = eval(jinit.options);
    		for(var i=0; i<json.length; i++)
			{
			...
			selectHtml += '</select>';//省
			selectHtml += '<select name="cityzoneNo" id="cityzoneNo" onchange="InitZoneNo(this);" class="required form-control" title="请选择城市"  >';
			selectHtml += '<option value="">---请选择---</option></select>';//市
			selectHtml += '<select name="zoneNo" id="zoneNo" class="required form-control" title="请选择区域">';
