...
Advice['default'] = {
	...
	reset: function(elm) {
		...
		each(types, function(type) {
			...
			if (data(adv, 'validation')) {	// created by framework
	...
	_getAdvice: function(elm, type) {
		...
		adv = elm.id ? ($('validation_advice_' + type + '_' + elm.id) 
				|| $('validation_advice_' + elm.id)) : null;
		adv = adv || $('validation_advice_' + type) || $('validation_advice');
		if (adv) {
			t[type] = adv;
	...
	_createAdvice: function(elm) {
		var id = 'validation_advice_' + (elm.id || new Date().getTime());
...
extend(Validation, {
	...
	validateElement: function(elm, options) {
		...
		v && v.call(elm, valid == -1, (valid == -2) ? 0 : valid);
	...
	validateForm: function(form, options) {
		...
		options.onFormValidate && options.onFormValidate.call(form, valid, 
				options.stopOnFirst ? errorElms[0] : errorElms);
	...
	_getValidators: function(elm, options) {
		return options.validators || options.validator || this._getValidatorsFromElm(elm, options);
	},
	...
	_getValidatorsFromElm: function(elm, options) {
		var vs = [];
	...
	_getMessage: function(elm, type, options, index) {
		...
		if (msg) {
			...
			msg = elm.getAttribute((options.messageNames || this.defaultOptions.messageNames)[type]);
			var useTitle = (options.useTitle === undefined) ? this.defaultOptions.useTitle : options.useTitle;
			if (!msg && type == 'failed' && useTitle) {
				msg = elm.title;
			}
			if (msg && type == 'failed' && index !== false) {
				var t = msg.split(options.messageSeparator || this.defaultOptions.messageSeparator);
				return t[index] || t[0];
			}
			return msg;
		}
	...
	_getAdvice: function(options) {
		...
		if (t === 'string') {
			...
			return Advice[this.defaultOptions.advice]
		}
	}
});
...
extend(Validation.prototype, {
	...
	_handleSubmit: function() {
		...
		var handler = bind(function(e) {
			var opts = {};
			if (options.onFormValidate) {
				opts.onFormValidate = function() {
					if (options.onFormValidate.apply(this, arguments) === false) {
						stopEvent(e);
					}
				};
			}
	...
	_handleElms: function() {
		...
		each(elms, function(elm) {
			var h = function() {
				return Validation._validateFormElement(elm, options);	// this code should be refactory.
			};
...
{
	var sDate = new Date(a.value.replace("-",",")).getTime();
	var endDate = new Date(b.value.replace("-",",")).getTime() ;
